...
namespace System.Windows.Forms
{
	...
	public abstract class TabStyleProvider : Component
	{
		...
		public static TabStyleProvider CreateProvider(CustomTabControl tabControl){
			...
			switch (tabControl.DisplayStyle) {
				case TabStyle.None:
					...
					break;
				case TabStyle.ManorDB:
					provider = new TabStyleManorDBProvider(tabControl);
					break;
		...
		//Custom variable for bookmarking
		//dictionary that refrences unique Tab name with the tab's color
		protected Dictionary<string, Color> _TabBookmarkColorsByName = new Dictionary<string, Color>();
		[System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Design", "CA1051:DoNotDeclareVisibleInstanceFields")]
		...
		public Dictionary<string,Color> TabBookmarkColorsByName
		{
			get
			{
				return this._TabBookmarkColorsByName;
			}
			set
			{
				this._TabBookmarkColorsByName = value;
				this._TabControl.Invalidate();
			}
		}
		...
		public void PaintTab(int index, Graphics graphics){
			using (GraphicsPath tabpath = this.GetTabBorder(index)) {
				using (Brush fillBrush = this.GetTabBackgroundBrush(index)) {
					...
					this.DrawTabBookmarkIndicator(tabpath, index, graphics);
		...
		//Going to repurpose the Focus Indicator to function as a bookmark for tabs so that they are more easily destinguished
		private void DrawTabBookmarkIndicator(GraphicsPath tabpath, int index, Graphics graphics)
		{
			string tabName = this._TabControl.TabPages[index].Name;
			if (this._TabBookmarkColorsByName.ContainsKey(tabName))
			{

				Color tabColor = this._TabBookmarkColorsByName[tabName];

				Brush bookmarkBrush = null;
				RectangleF pathRect = tabpath.GetBounds();
				Rectangle bookmarkRect = Rectangle.Empty;
				switch (this._TabControl.Alignment)
				{
					case TabAlignment.Top:
						bookmarkRect = new Rectangle((int)pathRect.X, (int)pathRect.Y, (int)pathRect.Width, 4);
						bookmarkBrush = new LinearGradientBrush(bookmarkRect, tabColor, SystemColors.Window, LinearGradientMode.Vertical);
						break;
					case TabAlignment.Bottom:
						bookmarkRect = new Rectangle((int)pathRect.X, (int)pathRect.Bottom - 4, (int)pathRect.Width, 4);
						bookmarkBrush = new LinearGradientBrush(bookmarkRect, SystemColors.ControlLight, tabColor, LinearGradientMode.Vertical);
						break;
					case TabAlignment.Left:
						bookmarkRect = new Rectangle((int)pathRect.X, (int)pathRect.Y, 4, (int)pathRect.Height);
						bookmarkBrush = new LinearGradientBrush(bookmarkRect, tabColor, SystemColors.ControlLight, LinearGradientMode.Horizontal);
						break;
					case TabAlignment.Right:
						bookmarkRect = new Rectangle((int)pathRect.Right - 4, (int)pathRect.Y, 4, (int)pathRect.Height);
						bookmarkBrush = new LinearGradientBrush(bookmarkRect, SystemColors.ControlLight, tabColor, LinearGradientMode.Horizontal);
						break;
				}

				//	Ensure the focus stip does not go outside the tab
				Region focusRegion = new Region(bookmarkRect);
				focusRegion.Intersect(tabpath);
				graphics.FillRegion(bookmarkBrush, focusRegion);
				focusRegion.Dispose();
